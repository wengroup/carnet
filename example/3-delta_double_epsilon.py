"""
Double contraction of a tensor with multiple delta tensors and double contraction with
epsilon tensor.
"""
...
def get_rules(rank: int, num_delta: int) -> list[str]:
    ...
    Returns:
        rules, e.g. ['xab,cd,ef,abcdefg->xg'], with deltas, epsilon, and the tensor
        indices in order.
    """
    ...
    for p in perms:
        ...
        for comb in itertools.combinations(remaining, 2):
            ...
            r = f"x{epsilon_contracted},{','.join(delta)},{t_indices}->x{uncontracted}"
            rules.append(r)
