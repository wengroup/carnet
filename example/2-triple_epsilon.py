"""
Find the linear dependence rules of triple contracting epsilon tensor with multiple
...
def get_rules(rank: int) -> list[str]:
    ...
    rules = []
    for comb in itertools.combinations(range(rank), 3):
        candidate = ["" for _ in range(rank)]

        # choose two positions to contract
        pos_a, pos_b, pos_c = comb
        candidate[pos_a] = "a"
        candidate[pos_b] = "b"
        candidate[pos_c] = "c"
        ...
        left = "".join(candidate)
        right = left.replace("a", "").replace("b", "").replace("c", "")

        rules.append(f"abc,{left}->{right}")

    return rules


def contract_triple_epsilon(t: Tensor, rules: list[str] = None) -> list[Tensor]:
    """
    Triple contract epsilon tensor with a tensor.
...
if __name__ == "__main__":
    ...
    candidates = contract_triple_epsilon(t, rules)
