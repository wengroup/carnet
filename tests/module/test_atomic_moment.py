...
def test_AtomicMoment(config_info):
    ...
    am = AtomicMoment(
        F=F, L1=L1, L2=L2, L3=L3, num_atom_types=num_atom_types, num_average_neigh=5.0
    )

    M = am(edge_vector, edge_idx, atom_type, x)

    # Check the total shape
    assert M.shape == (num_atoms, F, (3 ** (L3 + 1) - 1) // 2)

    # Check each l is a natural tensor
    for l3 in range(L3 + 1):
        sliced = M[..., (3**l3 - 1) // 2 : (3 ** (l3 + 1) - 1) // 2]
