    ...
    def __init__(
        ...
        Args:
            ...
            normalize: normalization method for the output tensor. Options are:
                `unity` or `none`. See `carnet.core.tp.tp_even`.
            path_mode: mode to construct the paths from L1 and L2 to L3. Options are:
                `full`, `camp`, or `lite`. See `carnet.module.utils.get_paths`.
    ...
    def forward(self, x: Tensor, y: Tensor, R: Optional[Tensor] = None) -> Tensor:
        ...
        for idx, l3 in enumerate(self.L3):
            ...
            if self.for_atomic_moment:
                ...
                z_l3_paths = torch.einsum(
                    "pabc, ...pF, ...Fb, ...c -> ...pFa", H_combined_l3, W_l3, x, y
                )
            else:
                # H_combined_l3: (Np, 3^l3, T1, T2)
                # x: (..., F, T1)
                ...
                # Result z_l3_paths: (..., Np, F, 3^l3)
                z_l3_paths = torch.einsum(
                    "pabc, ...Fb, ...Fc -> ...pFa", H_combined_l3, x, y
                )
