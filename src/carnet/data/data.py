...
from carnet._dtype import DTYPE_BOOL, DTYPE_INT, TORCH_FLOATS, TORCH_INTS
from carnet.data.neighbor import get_neigh
...
class Config(Data):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Shift vec is needed for potentials, because we need to compute edge_vector
...
@torch.jit.script
def get_edge_vec_single(
    pos: Tensor,
    shift_vector: Tensor,
    cell: Union[Tensor, None],
    edge_index: Tensor,
):
    """Get edge vector of a single config.

    Args:
        pos: (n_atoms, 3) array of atomic positions.
        shift_vector: (n_edges, 3) array of shift vectors.
        cell: (3, 3) array of lattice vectors.
    ...
    if cell is not None:
        edge_vec = edge_vec + shift_vector @ cell
...
def get_edge_vec_batch(
    ...
    Args:
        ...
        shift_vector: (n_edges, 3) array of shift vectors.
        cell: (n_graph, 3, 3) array of lattice vectors.
        edge_index: (2, n_edges) array of edge indices.
        batch: (n_nodes,) array of batch indices.
    ...
    if cell is not None:
        ...
        # (n_edges, 1, 3) @ (n_edges, 3, 3) -> (n_edges, 1, 3)
        pbc_shift = torch.bmm(shift_vector.unsqueeze(-2), expanded_cell).squeeze(-2)
        edge_vec = edge_vec + pbc_shift
